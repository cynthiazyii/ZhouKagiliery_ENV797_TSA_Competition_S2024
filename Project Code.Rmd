...
ETS_fit_test <-  stlf(TSDaily_train, h=365+181)
...
```{r}
frequency(FullTSTest)
```


```{r, echo=FALSE}
#TBATS autofit
FullTSTest <- na.approx(TSDaily_train)
tbats_model <- tbats(FullTSTest)
...
```{r}
h = length(TSDaily_test)
TBATS_forecast <- forecast(tbats_model, h = h)
```

```{r}
autoplot(TSDaily_train, series = "Original") +
  autolayer(TBATS_forecast, series = "TBATS forecast") +
  autolayer(TSDaily_test, series = "Test") +
  ggtitle("TBATS modeling")
```

```{r}
forecast_accuracyTBATS <- accuracy(TBATS_forecast)
print(forecast_accuracyTBATS)
```